...
export const ContactPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const kind: ContactKind = requestType === 'demo' ? 'walkthrough' : 'support';
  ...
  return (
    <Box sx={{ minHeight: '100vh' }}>
      <Seo
        title={t('seo.contactWalkthrough.title')}
        description={t('seo.contactWalkthrough.description')}
        path="/contact"
        ...
        jsonLd={[
          ...
          contactPage({
            ...
            name: t('seo.contactWalkthrough.title'),
            description: t('seo.contactWalkthrough.description'),
          }),
          breadcrumbList([homeCrumb(), { name: 'Contact' }]),
        ]}
      />
      <Header />

      <Box component="main" id="content">
        {/* Hero Section — every entry point (pricing, footer, FAQ, home,
            solutions) lands here framed as a walkthrough booking; the
            generic demo/support "Get in Touch" default was retired. */}
        <Section tone="hero" size="sm" sx={{ pt: { xs: 8, md: 12 }, pb: { xs: 5, md: 8 } }}>
          <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3 }}>
            <TierBadge featured>{t('contact.intent.walkthrough.chip')}</TierBadge>
          </Box>
          <SectionHeader
            level="h1"
            title={t('contact.intent.walkthrough.title')}
            description={t('contact.intent.walkthrough.subtitle')}
          />
        </Section>
        ...
        <Section size="md" maxWidth="md" sx={{ pt: { xs: 2, md: 3 } }}>
          ...
          <NeuPanel sx={{ p: { xs: 3, md: 5 } }}>
            {/* Request Type Toggle */}
            <Box sx={{ mb: 4 }}>
              {/* h6 for the size; h2 because it is the page's one sub-heading under the h1. */}
              <Typography variant="h6" component="h2" sx={{ mb: 2, fontWeight: 700, color: soft.text }}>
              ...
              >
                <ToggleButton value="demo">
                  ...
                  {t('contact.intent.walkthrough.toggle')}
                </ToggleButton>
