...
This is the practical tradeoff against [internal execution](/docs/execution-modes): with an
...
|---|---|---|
| **Endpoint** | The web address of your service. Required. | N/A |
...
| **Test value** | A representative value used when you test the endpoint, and prefilled if you later promote this function into a portfolio metric. It isn't used to guide the search itself. |
...
Right before Fintela actually calls your service, it rechecks where that address currently
points, including every destination it could resolve to. If any of them turns out to be a
private, internal, or otherwise nonpublic address, the call is refused before it's made. An
...
| Fintela's own code editor and live validation | There's no code on Fintela's side to edit or validate; your logic lives entirely on your own service. |
...
| Ranges, bounds, or multiple choice parameters | A fitness parameter is just a name, a type, an optional description, and a test value; its value stays fixed for the whole study. |
