...
|---|---|---|
| Endpoint address | The address of your service that Fintela calls | N/A |
...
Fintela can generate this for you automatically and keep it in sync as you add or rename parameters, or you can customize it yourself: useful when your warmup need isn't a simple function of your parameters.
...
> [!WARNING]
> If your strategy chains several calculations that each need their own warmup, add the days together for calculations that happen one after another, and use the largest for calculations that happen side by side.
...
6. Any built in data source that needs its own warmup has one.
...
| Warmup (lookback) function | Required | Required |
