...
| You picked a metric that needs a benchmark, on a study with none set | pick a metric that doesn't need one, or set a benchmark and rerun |
...
> `High Risk`, `Controlled Risk`, `Train≫OOS`, `Val Weak`, `OOS Strong`, `Overall Best`, `Weak` and `Stable` on a candidate's Performance tab are simple onscreen comparisons between periods. They're useful diagnostics, but the statistically grounded answer is always the Robustness tab's verdict.
