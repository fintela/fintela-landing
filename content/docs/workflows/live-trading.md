...
Creating an operation runs a light check; **launching** runs the full one; and **resuming** reruns the safety critical part of it. If a check fails, you'll see a message naming exactly what to fix: nothing is created or started partially.

| Check | Fails when | Create | Launch | Resume |
|---|---|---|---|---|
| Connection | The connection isn't active, or isn't yours | yes | N/A | N/A |
| Capital | The capital amount isn't greater than zero | yes | N/A | N/A |
| Capital limit | The capital amount exceeds this connection's trading limit | N/A | yes | N/A |
| Duplicate | The group already has an operation on this connection | yes | N/A | N/A |
| Membership | The group has no active members | yes | yes | N/A |
| Execution mode | A member's strategy runs outside Fintela's daily automatic updates | yes | yes | N/A |
| Daily update | Daily update is switched off on the group | yes | yes | yes |
| Daily schedule | A member isn't scheduled for daily updates | N/A | yes | yes |
| Freshness | A member's data is out of date | N/A | yes | yes |
| Meta portfolios | A portfolio built from other portfolios hasn't yet been flattened into individual holdings | N/A | yes | yes |
| Shorts | A member holds a short crypto position, or a short equity position on an account not enabled for short selling | N/A | yes | yes |
| Endpoint security | A member's external risk manager can only be reached over an unencrypted connection | N/A | yes | yes |
| Execution config | The order type or time in force you've chosen isn't supported for rebalancing | yes, for a per operation override | yes, for the effective setting | N/A |
