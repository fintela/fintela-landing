...
| Decision | What it affects |
|---|---|
| **Built in** vs **Internal** vs **External** | Built in options are ready made, standard metrics (Sharpe ratio, CAGR and the like) with nothing to configure and nothing to edit. Internal and External work like strategies do: you write the scoring logic yourself, or point to logic hosted on your own systems, and that choice is locked in once made |
...
> A study optimizes toward exactly one number, in exactly one direction: there's no multiobjective or tradeoff mode. Rule based, no code fitness functions aren't available yet either, so plan on writing one in code if a built in doesn't cover what you need.
...
| Where you pick a built in | Built in options don't show up in this registry's list: you choose one directly while setting up a study at stage 5 |
...
> [!WARNING] Promoting locks in a snapshot: it doesn't rerun anything
...
2. From the study's row in the registry, choose **Duplicate**. This opens the study builder prefilled with the same setup, under a new name, ready to edit: nothing is created yet.
