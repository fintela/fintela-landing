...
| Holdings | What the portfolio held, and when | N/A |
...
| Invert (what if) | Flips every position long to short and short to long and reruns the simulation, purely as a preview: nothing is saved or traded. |
...
| Avg Duration | Average holding period for closed trades | N/A |
...
| Situation | What you see |
|---|---|
| Scores not computed yet | "No robustness analysis available for this portfolio yet. Scores are computed when a study finishes: rerun the study (or its finalization) to populate them." |
...
**Deflated Sharpe** is a 0 to 100% gauge (your confidence that this is real skill, not luck) coloured red below 90%, amber from 90 to 95%, and green at 95% and above. An outer ring shows the study wide overfitting probability, turning red once it passes 50%. With no deflated Sharpe available, it reads "Not enough data to compute a Deflated Sharpe."
