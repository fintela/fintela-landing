...
| [Portfolio groups](/docs/portfolio-groups) | No | N/A |
| [Promoted portfolios](/docs/promoted-portfolios) | No | N/A |
...
| A `required_lookback(...)` warmup function | Required | Not applicable | Optional |
...
| Lookback logic changed since validating | Validate again so your warmup window is proven for the current code. |

Only strategies check the exact parameter values, since only a strategy's timing and warmup
...
You still need to provide a `required_lookback(...)` warmup function for an external strategy,
...
to call it if it points at a private, internal, or otherwise nonpublic network location. Fintela
