...
| QAOA (Quantum Optimization, experimental) | No (runs on a single worker) | 50 to 500 |
| Quantum Kernel Bayesian Optimization (experimental) | No: runs on a single worker | 30 to 300 |
...
It shares QAOA's safety behavior: a random warmup period before its model engages, and an
...
Neither quantum sampler has any per study settings you can adjust: there's no dial for warmup
...
though genetic algorithms like it are often used for multiobjective optimization elsewhere,
Fintela always runs it against your one objective. If you need to balance two things against each
other (say, return against drawdown) build that tradeoff into a single custom
[fitness function](/docs/fitness-functions) rather than looking for a multiobjective mode.
...
and **Quality**, using simple 1 to 4 or 1 to 3 scales (● = high, ◌ = low), plus a **Best for** column:
...
A banner in the dialog sums up the tradeoff: *Faster samplers (Random, QMC) scale perfectly across
